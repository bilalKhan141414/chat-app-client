...
export default function useApi() {
    ...
    useEffect(() => {
      ...
      axiosNumber = axiosInstance.interceptors.request.use((config) => {
        if (Helpers.isUserLoggedIn()) {
          config.headers.Authorization = `${Helpers.getAuthToken()}`;
        }
    ...
    useEffect(() => {
      axiosInstance.interceptors.response.use(undefined, (err) => {
        ...
        if (
          config.headers[RetryConstants.RetryCount] >= config.headers[RetryConstants.Retry]
        ) {
    ...
    const postRequest = useCallback(({url, data, config = {}}, showProgress = false) => {
        if(showProgress)
        config.onUploadProgress = HandleProgressUpload;

        return axiosInstance.post(`${window.ENDPOINT}${url}`, data, config)
        .then(getResponseData)
        .catch(throwError);
    }, [throwError])
    
    const getRequest = useCallback((path, extraConfig = {}) => {
    ...
    const putRequest = useCallback(async (path, data, config = {}) => {
      return axiosInstance
    ...
    const deleteRequest = useCallback(async (path, id, config = {}) => {
      return axiosInstance
